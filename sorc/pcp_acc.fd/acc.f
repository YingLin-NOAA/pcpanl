...
! 2016-09-28  Y. Lin:      - If files to be summed do not have the same 
!                            dimensions, do not proceed with putgb.  For
!                            some reason the gross mis-match during summing
!                            trips up putgb - it just writes out an empty
!                            accumulated file, without setting iret to either
!                            zero or non-zero (it probably just exits silently).
!                            This pre-caution is case an RFC sends out wrong 
!                            QPEs, as happened on 27 Sept 2016.
!
!    Unit 5 input cards:
!     ----:----|----:----|
...
!
        ...
        write(6,*) 'getgb ', fname, ' irets=', irets, ' kf=', kf
...
!   have needed date info
           kpdso=kpds
           kgdso=kgds
!   save the (1-d) dimension of the first file.  All subsequent files
!   should have the same dimension (this is to guard against the rare 
!   occasion when an RFC sends a file of a different dimension (27 Sept 2016),
!   and totally muck things up).
           kf1=kf
          else
           if (kf.ne.kf1) then
             aok=.false.
             write(6,*) 'Array dimensions differ!  Will not do acc.'
           endif
          endif
